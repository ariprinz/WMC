...
// Eulersche Pfade bzw. Zyklen prüfen
...
function displayAnalysisResults(results) {
    ...
    // Distanzmatrix formatieren mit Zeilenumbrüchen
    const formattedDistances = results.distances.map(row => 
        row.map(d => d === Infinity ? '∞' : d).join('; ')
    ).join('\n');
    
    container.innerHTML = `
        <div class="analysis-card">
            <h4>Distanzmatrix</h4>
            <div class="analysis-content" style="white-space: pre-line;">${formattedDistances}</div>
        </div>
        
        <div class="analysis-card">
            <h4>Exzentrizitäten</h4>
            <div class="analysis-content" style="white-space: pre-line;">${results.eccentricities.map((e, i) => 
                `Knoten ${i}: ${e === Infinity ? '∞' : e}`).join('\n')}</div>
        </div>
        
        <div class="analysis-card">
            <h4>Radius, Durchmesser und Zentrum</h4>
            <div class="analysis-content" style="white-space: pre-line;">Radius: ${results.radius === Infinity ? '∞' : results.radius}
...
Zentrum: {${results.center.join(', ')}}</div>
        ...
        <div class="analysis-card">
            <h4>Komponenten</h4>
            <div class="analysis-content" style="white-space: pre-line;">${results.components.map((comp, i) => 
        ...
        <div class="analysis-card">
            <h4>Brücken</h4>
            <div class="analysis-content" style="white-space: pre-line;">${results.bridges.length > 0 ? 
