...
function showLoading(containerId) {
    ...
    if (element) {
        element.innerHTML = '<div class="loading">Loading Films</div>';
    }
}
    ...
    try {
        ...
        if (data && data.results && data.results.length > 0) {
            const moviesHtml = data.results
                ...
                .map(createMovieCard)
                .filter(card => card) // Leere Karten werden entfert
                .join('');
...
async function searchClownMovies() {
    ...
    try {
        ...
        // Duplikate werden entfert und nach Bewertung sortiert
